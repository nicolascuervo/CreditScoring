...
def score_model(y_true: np.array,
    ...
    approved = np.int64(y_pred_proba < validation_threshold)
    
    max_loss = cost_ratio*sum(y_true)
    max_gain = 1.0*sum(1-y_true)

    losses = cost_ratio * sum( approved*y_true )
    gains =  1.0*sum( approved * (1 - y_true))

    total_score = (gains - losses + max_loss)/(max_gain+max_loss)

    if verbose:        
        print('approved:', approved, sep='\t')
...
def record_model_run(model: Any,
    ...
    model_score = score_model(test_tupple[1].values, y_pred, validation_threshold=validation_threshold)
...
def plot_roc_curve(pred_test_pairs: Dict[str, Tuple[np.array, np.array]]) -> None:
    ...
    for model_name, (y_tst, y_prd, validation_threshold) in pred_test_pairs.items():
        ...
        model_score = score_model(y_tst, y_prd, validation_threshold=validation_threshold, verbose=False)
        
        # Plot the ROC curve for the model
        plt.plot(fpr, tpr, label=f'{model_name}; score={model_score:0.4f}')
